...
def _fallback_when_empty_per_row(
    primary: pd.Series,
    fallback: pd.Series | None,
) -> pd.Series:
    if fallback is None:
        return primary
    empty_mask = primary.apply(_is_empty_rate_card_value)
    return primary.where(~empty_mask, fallback)


def _resolve_thc_currency_series(
    cost_series: pd.Series,
